    ...
    fun parse(
        ...
        if (node.isBoolean) {
            ...
            SourceObjectSchema(
                ...
                dynamicAnchor = node["\$dynamicAnchor"]?.takeIf(JsonNode::isTextual)?.textValue(),
                recursiveAnchor = node["\$recursiveAnchor"]?.asBoolean() == true,
                types = readTypes(node, version),
                staticReference = node["\$ref"]?.takeIf(JsonNode::isTextual)?.textValue(),
                dynamicReference = node["\$dynamicRef"]?.takeIf(JsonNode::isTextual)?.textValue(),
                recursiveReference = node["\$recursiveRef"]?.takeIf(JsonNode::isTextual)?.textValue(),
